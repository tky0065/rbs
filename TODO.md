...
- [x] **S4** · Le noyau cesse de nommer PostgreSQL — vérifié 2026-08-28 · `cargo test --workspace --all-features -- --ignored` → **31 passed, exit 0**, dont `rbs doctor` joué sur un projet de chacun des trois moteurs et rendant `✓ base   mysql 8.4 répond sur …` sans nommer PostgreSQL ailleurs que sur PostgreSQL — **l'assertion porte sur la ligne rendue et non sur le code de sortie**, `doctor` sortant en 0 même quand un contrôle échoue, ce qu'une première version du test n'avait pas vu ; morsure **plancher MySQL relevé à 99.0** → « ✗ base   mysql 8.4 sur 127.0.0.1:55443, 99.0 attendu au minimum », **sur MySQL seul** · `cargo test -p rbs-core db::` → 7 passed, un message par moteur, SQLite recevant une phrase d'une autre nature — il n'a pas de serveur à démarrer mais un fichier à rendre accessible · **le moteur se lit dans le schéma de l'URL** et non dans `[package.metadata.rbs]` : le noyau est une bibliothèque, le manifeste qui l'emploie ne lui appartient pas ; le helper reste privé, rien de public ne s'ajoutant à une API que `v1.0` gèlera · **`doctor` cessait de fonctionner hors PostgreSQL bien avant de le nommer** : la commande `version` du binaire engendré exécute `SHOW server_version_num`, que ni MySQL ni SQLite ne comprennent — elle prend trois branches, et `doctor` interprète leurs trois formes, « 8.0.36-0ubuntu0.22.04.1 » compris · **un plancher par moteur, chacun avec sa cause** : PostgreSQL 14 pour le support communautaire, MySQL 8.0 pour `FOR UPDATE SKIP LOCKED`, SQLite 3.35 pour `UPDATE … RETURNING` — les deux dernières étant ce dont le dépilage de `S3` dépend, et la feature `sqlite-use-returning-for-3_35` de SeaORM corroborant la troisième · **constat traité sur arbitrage, hors des deux critères** : le noyau héritait un `sqlx-postgres` du workspace, et l'unification des features de Cargo faisait compiler PostgreSQL à un projet MySQL ; `rbs-core` expose désormais `postgres`/`mysql`/`sqlite`, `cargo build -p rbs-core --no-default-features --features <pilote>` aboutissant pour les trois pris isolément
...
- [x] **T1** · Exemple `newsletter-queue` compilé en CI — vérifié 2026-08-28 · `cargo clippy --workspace --all-targets -- -D warnings` dans les **quatre** exemples, la boucle exacte du step `examples/`, → Finished pour les quatre ; `cargo test -p rbs-cli --test integration_examples` → **16 passed**, dont `newsletter_queue_is_what_the_cli_produces_today` · morsure `jobs::enqueue(db, …)` au lieu de `&transaction` → seul `the_hand_edits_of_newsletter_queue_are_in_place` tombe, les quinze autres restent verts : la comparaison de dérive ne peut pas voir un fichier qu'elle exclut, et c'est ce test qui couvre les onze · **`mail` retenu, contre la parenthèse de la tâche** : le step `examples/` ne lance que clippy — ni test ni service — et `file-drop` porte déjà `mail` sans conteneur, donc le prix annoncé n'existe pas ; et `T2` l'exige, sa page de `mail` devant « montrer le passage à un job » sans écrire d'extrait à la main · **défaut du gabarit `jobs` mis au jour** : `&format!(…)` passé à `Statement::from_sql_and_values` faisait échouer `clippy -D warnings` sur **tout** projet ayant installé `jobs` — la vérification du portage sur trois moteurs compilait le projet engendré sans le passer à clippy, et aucun test ne franchissait les deux à la fois ; corrigé, `integration_jobs -- --ignored` → 3 passed en 42,87 s · la CI n'a pas bougé, sa boucle sur `examples/*/` prenant le nouveau venu d'elle-même
...
- [x] **T2** · Pages de documentation FR et EN — vérifié 2026-08-28 · `node scripts/parite.mjs` → **22 paires de pages, 0 écart structurel** ; les 36 blocs `file=` des pages écrites citent tous `examples/newsletter-queue`, aucun autre exemple ; `npm run build` → succès pour les deux locales, ce qui résout chaque `file=` et chaque `region=`, et morsure `region=registry` renommée → le build tombe sur « La région « registre » est introuvable dans examples/newsletter-queue/src/jobs/mod.rs » · **troisième critère validé par le user à la lecture**, la section montrant les trois clauses : le compromis énoncé (ni file ni réessai — SMTP indisponible, DNS, redémarrage), le passage montré par deux extraits, et `send_detached` conservé sous une permission propre · **périmètre porté au jalon entier sur arbitrage** : `rbs dev` et `--database` n'apparaissaient nulle part dans les dix-huit pages, `rbs seed` une seule fois — six pages en deux langues · **la page de `mail` migre de `file-drop` vers `newsletter-queue`** : chaque page du site ne cite qu'un seul exemple, sans exception sur les dix, et c'est la seule lecture qui tienne les critères 2 et 3 ensemble · **trois blocs de la page `doctor` étaient faux et sont recapturés** — sept points d'insertion là où il y en a neuf, « PostgreSQL 18.6 » là où la ligne nomme le moteur, un plancher unique là où il y en a un par moteur · **le commentaire d'`integration_doctor` affirmait que `doctor` sort en 0 quand un contrôle échoue : mesuré à 1**, `diagnose` rendant `report.succeeded()` — les tests n'en dépendaient pas, assertant sur la ligne rendue, mais la raison écrite était fausse et est corrigée ; la ligne de `S4` porte la même affirmation et n'a pas été réécrite
...
- [x] **T3** · `doctor` diagnostique les jobs et la base — vérifié 2026-08-28 · `cargo test -p rbs-cli --test integration_doctor` → **2 passed**, relancé par moi dans le worktree avant de cocher, puis les deux constats **reproduits à la main** avec le binaire livré : `✗ jobs   config/default.toml ne porte pas de section « [jobs] »` suivi des trois réglages à coller, et `✗ base   le manifeste compile « sqlx-postgres » et RBS_DATABASE__URL est une URL « mysql:// »` · morsure garde de l'écart désarmée → seul `a_driver_at_odds_with_the_url_is_named_on_both_sides` tombe, sur « ✗ base rien ne répond sur 127.0.0.1:1 » — le port injoignable masquait le vrai constat · **les assertions portent sur la ligne rendue et jamais sur le code de sortie**, `doctor` sortant en 0 même quand un contrôle échoue · **ni conteneur ni compilation du projet engendré** : les deux fautes se lisent dans le manifeste et la configuration, et un diagnostic qui exigerait la base qu'il diagnostique ne servirait à personne · l'écart se dit **avant** le sondage du port, un serveur qui répond ne prouvant rien quand le pilote compilé ne parle pas son protocole · le moteur est lu de la feature `sea-orm` de `[dependencies]`, seule réellement embarquée, et non de `[package.metadata.rbs].database` qu'une édition à la main laisse derrière
      ✓ `jobs` déclarée sans section `[jobs]` → `✗`.
      ✓ URL `mysql://` avec `sqlx-postgres` au manifeste → `✗` nommant l'écart.

- [x] **T4** · Critère de sortie du jalon — vérifié 2026-08-28 · `cargo test -p rbs-cli --test integration_new each_engine -- --ignored` → **1 passed en 49,78 s**, PostgreSQL et MySQL en conteneur, SQLite sur fichier, chacun dans sa cible : `rbs new --database <moteur>`, `generate crud`, `migrate up`, puis `cargo test` du projet engendré · `cargo test -p rbs-cli --test integration_jobs -- --ignored` → **3 passed en 95,09 s**, dont `a_job_enqueued_before_the_process_is_killed_runs_after_the_restart` · `git diff --stat -- crates/rbs-core/` → **0 ligne** sur `7a6c782..51124fb` (`P`+`Q`+`R`) et **0 ligne** sur `0b39bb0..HEAD` (lot `T`), contre 76 insertions et 4 suppressions sur `51124fb..0b39bb0` : seul `S` a ouvert le noyau · **le test des trois moteurs s'arrêtait à `cargo build`**, qui ne demande aucune base — les requêtes qu'engendre SeaORM pour un moteur ne sont exercées qu'à l'exécution ; il monte à `migrate up` puis `cargo test` · **la première version était vide de sens et passait au vert** : un projet vierge n'a aucun test qui touche la base et `cargo test` y rend « 0 passed » sur les trois moteurs, y compris sur un moteur dont pas une requête ne fonctionnerait — le test engendre donc un CRUD et **exige nommément** ses quatre tests en `... ok` ; morsure du CRUD engendré sous un autre nom → « `articles::tests::the_full_lifecycle_goes_through_the_api` n'a pas tourné sur postgres », sur un rapport pourtant vert ; morsure `migrate up` → `migrate status` → la suite tombe sur les trois · les démarreurs de conteneurs remontent d'`integration_jobs` vers `common/`, deux fichiers de tests les employant désormais
...
- [x] **U1** · Workflow de publication sur tag — vérifié 2026-08-28 · les deux critères prouvés **en conditions réelles** sur le run du tag `v0.4.0` : steps « garde de version » et « cargo publish (dry-run) » verts, puis « cargo publish » vert · en local, `garde-version.sh v0.4.0` sur un workspace en 0.1.0 → sortie 1 nommant les deux crates puis « publication refusée, aucune crate n'a été envoyée », et la garde change de verdict dans les deux sens après le passage en 0.4.0 — elle lit le dépôt, elle n'est pas une constante · un step de `ci.yml` la mord sur ses deux verdicts à chaque commit, en dérivant la version au lieu de la coder en dur · la limite non traitée tient : `rbs-cli` échouant après un `rbs-core` publié, le run n'est pas rejouable tel quel · le dry-run de `ci.yml` aligné sur `--locked`, qui résolvait librement quand la release exige un verrou intact
...
- [x] **U2** · `CHANGELOG.md`, et le README remis à jour — vérifié 2026-08-28 · `npm run parite` → **4 paires de fichiers racine, 0 écart structurel**, 18 liens résolus, et les 22 paires du site inchangées · l'instrument manquait et a été étendu : il ne comparait que les pages du site quand le critère porte sur la racine · éprouvé avant de servir de preuve, trois morsures **relancées par moi** rendant chacune son diagnostic — titre retiré du CHANGELOG français → « niveau de titre : 4 en anglais, 3 en français », `SECURITY.md` posé seul → « aucune version française », renvoi du français vers l'anglais → « README.fr.md : « CODE_OF_CONDUCT.md » renvoie vers la version anglaise » · sortie 1 sur écart, 0 sinon · `grep` des titres du CHANGELOG → seuls `[Unreleased]` / `[Non publié]`, **aucun numéro de version**, et `git tag -l` → 0 · **réserve sur la mesure** : le « même dernier commit » de `V2` n'est pas appliqué aux paires racine, les moitiés françaises de `CONTRIBUTING` et `CODE_OF_CONDUCT` étant nées en `V2` et non avec leur original — le contrôle serait rouge à perpétuité ; la contrepartie posée est le contrôle « renvoie vers la version anglaise », qui a trouvé une brèche réelle dans `README.fr.md` · deux corrections hors critère, le README passant à son état d'après-publication : « five commands » → sept, et « PostgreSQL 18 » → les planchers réels de `doctor` (PostgreSQL 14, MySQL 8.0, SQLite 3.35)
...
- [x] **U3** · Publication réelle de 0.4.0 — vérifié 2026-08-28 · **`rbs-core 0.4.0` et `rbs-cli 0.4.0` sont sur crates.io**, run du tag `v0.4.0` vert sur tous ses steps · `cargo install rbs-cli` **sans `--git`** → « Replaced … with `rbs-cli v0.4.0` », `~/.cargo/bin/rbs --version` → `rbs 0.4.0` · `rbs new preuve-publication --yes` **sans `--core-path`** → `rbs-core = { version = "0.4.0" }` sans aucun `path`, `cargo build` → Finished, et le verrou porte `source = "registry+…crates.io-index"` avec checksum : résolu depuis le registre · `rbs doctor` sur ce projet → **`✓ versions   projet et rbs-core 0.4.0 alignés sur le CLI 0.4.0`**, le noyau publié n'est plus diagnostiqué en échec · **deux échecs traversés** : le contrôle de non-dérive a rattrapé les quatre exemples restés en `[package.metadata.rbs].version = "0.1.0"` avant tout tag — premier cas d'usage de `rbs upgrade`, fait à la main ; puis le premier run a rendu « status 400 : A verified email address is required », le compte crates.io n'ayant pas d'e-mail vérifié — l'échec est tombé sur `rbs-core` **avant** `rbs-cli`, donc rien n'était parti et le rejeu du même tag a suffi · **la friction de `V1` est levée** : le parcours nominal du README n'exige plus de contournement
...
- [x] **W2** · `#[non_exhaustive]` sur ce qui grossira — vérifié 2026-08-28 · **22 items posés, 3 exclus**, catalogue arrêté sur mesures et justifié item par item dans le commit · l'inventaire réel corrige la conception : **7 enums et non 5** (`LogError` et `LogFormat` s'étaient ajoutés) et 18 structs · exclusions, chacune sur une construction du code engendré qui tomberait : `Claims` (`auth/service.rs.jinja:124`, seul type du noyau construit littéralement), `ValidatedJson<T>` (déstructuré dans chaque contrôleur — l'attribut interdit aussi le filtrage), `CommonResponses` (`modifiers(&CommonResponses)`, struct unité instanciée) · **`Page<T>` reçoit l'attribut contre ce que la conception laissait attendre** : elle le donnait pour construit à chaque liste, la mesure montre qu'il passe par `Page::new` · les quatre exemples compilent **sans une ligne touchée**, `git diff` sur `examples/` vide · troisième critère prouvé **en deux temps par moi**, le second ne démontrant rien sans le premier : sans l'attribut, la variante ajoutée rend « failure enum_variant_added: enum variant added on exhaustive enum · variant Error:SondeDeMesure » ; avec, « 196 checks: 196 pass · no semver update required » · **RÉSERVE ASSUMÉE, la CI est rouge jusqu'à `Y2`** : `semver-checks` compare à la 0.4.0 publiée, qui ne porte pas l'attribut, et signale la pose comme rupture majeure (sortie 100) — c'est la vérité, la rupture existe et n'est pas publiée · monter le workspace à 1.0.0 rendrait la CI verte mais désarmerait l'instrument (`0 checks: 0 pass, 254 skip`, tout étant permis en montée majeure), et le numéro appartient à `Y2` · arbitrage : la CI dit vrai plutôt que d'être verte
...
- [x] **W3** · La politique semver, README et docs FR/EN — vérifié 2026-08-28 · les deux README annoncent « rbs follows semantic versioning from 1.0 on » / « rbs suit le versionnage sémantique à partir de la 1.0 », **aucune trace résiduelle** de l'ancienne formule dans les README ni sur le site · `docs/docs/compatibility.md` et son miroir français énoncent les cinq périmètres de la conception §2.4, avec les mêmes verdicts de part et d'autre — **vérifié à la main, ligne à ligne** · `npm run parite` → 23 paires de pages (22 avant), 57 liens, 4 paires racine, **0 écart** · `npm run clear && npm run build` → deux `[SUCCESS]`, `compatibility/index.html` et `fr/compatibility/index.html` engendrées, `onBrokenLinks: throw` validant les renvois au passage · **limite de l'instrument, trouvée en le mordant** : un titre retiré du français est signalé (« compatibility.md : niveau de titre : 6 en anglais, 5 en français »), mais **une ligne de tableau retirée ne l'est pas** — `parite.mjs` compare la charpente des titres, les blocs, les encarts et les liens, jamais le contenu des tableaux ; la présence des cinq périmètres dans les deux langues a donc été vérifiée hors instrument · le pointeur `README.md:14` du backlog était périmé, `U2` ayant décalé la phrase en ligne 16 · portée élargie et assumée : `docs/intro.md` et son miroir annonçaient encore « no semver promise before 1.0 » en page d'accueil, contredisant la page neuve — repris dans le même commit
...
- [x] **X2** · Les notes de migration embarquées — vérifié 2026-08-28 · note de **16 lignes**, `crates/rbs-cli/notes/1.0.0.md`, embarquée par `include_dir` et **emportée par le paquet** (`cargo package --list` → `notes/1.0.0.md`) · saut 0.4.0 → 1.0.0 exercé sur le binaire recompilé en 1.0.0 : la note s'affiche après l'application du plan et dit la rupture réelle — le bras `_ =>` sur les 7 enums, la construction littérale interdite, et les trois exclus « c'est délibéré … un projet engendré traverse la version sans une ligne à changer », ce que la compilation des quatre exemples avait mesuré en `W2` · saut 1.0.0 → 1.1.0 → « aucune note de migration pour rbs 1.0.0 → 1.1.0 », **sortie 0**, non une erreur · **le test de complétude passe à vide aujourd'hui** (`PUBLIEE` = `CLI` = 0.4.0) et a donc été **mordu par moi** en simulant le futur — workspace en 1.0.0 et note retirée : « rbs 0.4.0 → 1.0.0 romprait sans le dire : écrivez crates/rbs-cli/notes/1.0.0.md », la note remise → 7 passed · décision de conception : une note appartient à la **version qu'elle introduit**, non à un couple, sans quoi la rupture d'une version enjambée disparaîtrait — `traversees("0.4.0", "2.0.0")` rend bien celle de 1.0.0 · `cargo test -p rbs-cli --lib` 539 passed, `integration_upgrade` 5 passed, les quatre comportements d'`X1` intacts
...
- [x] **X3** · `doctor` renvoie vers `rbs upgrade` — vérifié 2026-08-28 · sorties capturées sur le binaire, dans les trois directions · projet en retard → « ✗ versions   projet généré par rbs 0.0.1, CLI 0.4.0 » puis le conseil « lancez `rbs upgrade` : il aligne le manifeste du projet sur rbs 0.4.0 » · projet aligné → « ✓ versions   projet et rbs-core 0.4.0 alignés sur le CLI 0.4.0 », **mot pour mot la ligne capturée en prouvant `U3`**, avant que cette tâche n'existe : elle n'a pas bougé · projet en avance → « relancez la commande avec le CLI qui a généré le projet », sans nommer une commande qui refuserait ce sens · **le conseil vérifié en le suivant** : `doctor` signale → `upgrade` répond « ✓ manifeste aligné sur rbs 0.4.0 » → `doctor` repasse au `✓` → `git diff --name-only` rend `Cargo.toml` seul · la comparaison de versions est celle d'`upgrade`, rendue `pub(crate)` plutôt que réécrite, deux règles divergentes pouvant conseiller une commande qui refuse · `cargo test -p rbs-cli` et `clippy -D warnings` propres · TDD : rouge observé avant l'implémentation pour le premier critère, pas pour le second — l'agent le signale, un test de non-régression ne pouvant pas échouer avant
...
- [x] **Y1** · Page `cli/upgrade`, FR et EN — vérifié 2026-08-29 · `npm run parite` → **24 paires de pages** (23 avant), 60 liens, 4 paires racine, **0 écart**, sortie 0 ; `npm run clear && npm run build` → deux `[SUCCESS]`, `cli/upgrade/` et `fr/cli/upgrade/` engendrées · **8 blocs de terminal, tous capturés** : `--help`, saut avec note, `git diff --name-only`, `git diff -U1`, saut sans note, « rien à faire », refus d'un projet postérieur, refus d'un arbre sale — le synopsis **rejoué par moi** rend une sortie **mot pour mot identique** à la page · le seul bloc non capturé qu'il avait écrit a été retiré et passé en prose · deux morsures de l'instrument, chacune nommant le fichier (« cli/upgrade.md : niveau de titre : 9 en anglais, 8 en français », « lien relatif nº2 : « ../compatibility.md » … « ../architecture.md » ») · la limite des tableaux ne mord pas ici : la page n'en porte aucun, écrit délibérément en prose pour ne pas introduire une correspondance que l'instrument ne mesure pas · workspace monté à 1.0.0 pour deux captures puis restauré, `Cargo.toml` et `Cargo.lock` **identiques au MD5** · **correction jointe, hors critères** : trois affirmations fausses du site reprises dans les deux langues — plancher d'installation 18 → 14, « PostgreSQL 18 is the floor since uuidv7() became native » réécrit (le modèle pose l'identifiant, d'où la même migration sur les trois moteurs), et « a PostgreSQL 18 container … not negotiable » → 17, ce que `tests/common/mod.rs:160` déclare ; les sorties capturées (`✓ base PostgreSQL 18.6 répond`) laissées intactes · **un commentaire démenti par son propre code** corrigé dans la foulée : `generate/bench.rs` annonçait un conteneur 18 « non négociable » quatre lignes au-dessus d'un `GenericImage::new("postgres", "17")`
...
- [x] **Y3** · Critère de sortie du jalon — vérifié 2026-08-29 · **la boucle complète, jouée avec les deux versions du registre** : `cargo install rbs-cli --version 0.4.0` → projet `boutique` engendré par elle, `rbs add auth` (9 fichiers) et `rbs generate crud produit` (9 fichiers), soit **18 fichiers de code métier et deux suites de tests** ; puis `cargo install rbs-cli --version 1.0.0` → `rbs upgrade` → plan affiché, note de migration affichée, `migrate up` sur PostgreSQL **14.24** en conteneur, `cargo test` → **25 passed, 0 failed** · **la promesse de la note est tenue au bit près** : empreinte `shasum` des sources identique avant et après (`f741373b…`), `git diff --name-only` → `Cargo.toml` seul · un premier essai sur un projet nu avait rendu « vert » avec **0 test** — refait avec du code réel plutôt que compté comme preuve · `cargo semver-checks` compare désormais 1.0.0 à 1.0.0 et fait **223 contrôles contre 196 avant le gel**, tous verts : l'instrument est réarmé et en vérifie davantage, les `#[non_exhaustive]` ouvrant des lints que la surface non gelée ne permettait pas ; avant publication il rendait `major change`, cohérent avec le numéro · `README.md:16` et son homologue français annoncent « rbs follows semantic versioning from 1.0 on » / « rbs suit le versionnage sémantique à partir de la 1.0 », vérifié cette fois contre une 1.0.0 réellement parue
