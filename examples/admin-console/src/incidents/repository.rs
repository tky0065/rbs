...
use super::filter::{self, IncidentFilter};
...
pub async fn list(db: &DatabaseConnection, pagination: &Pagination) -> Result<(Vec<Model>, u64)> {
    ...
    filter(db, &IncidentFilter::default(), pagination).await
}
...
pub async fn filter(
    ...
    let (incidents, total) = tokio::try_join!(page, requete.count(db))?;
...
pub async fn create(db: &DatabaseConnection, incident: ActiveModel) -> Result<Model> {
    incident.insert(db).await.map_err(conflict_on_duplicate)
}

pub async fn update(db: &DatabaseConnection, incident: ActiveModel) -> Result<Model> {
    incident.update(db).await.map_err(conflict_on_duplicate)
}
