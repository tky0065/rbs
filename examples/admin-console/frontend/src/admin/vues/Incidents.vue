...
const TEXTES = {
  titre: 'Incidents',
  sous_titre: 'Les lignes de la table incidents, lues et écrites par les routes que ce projet expose.',
  filtre: 'Filtrer',
  lignes: 'lignes',
...
function corps(formulaire: Formulaire) {
  return {
    ...
    gravite: formulaire.gravite as 'basse' | 'moyenne' | 'haute',
    ouvert: formulaire.ouvert,
    duree_minutes: formulaire.duree_minutes === '' ? null : Number(formulaire.duree_minutes),
    echeance: formulaire.echeance === '' ? null : formulaire.echeance,
    constate_le: new Date(formulaire.constate_le).toISOString(),
  }
}
...
function depuis(recu: IncidentResponse): Ligne {
  return {
    id: recu.id,
    reference: recu.reference,
    sujet: recu.sujet,
    detail: recu.detail,
    gravite: recu.gravite,
    ouvert: recu.ouvert,
    duree_minutes: recu.duree_minutes ?? null,
    echeance: recu.echeance ?? null,
    constate_le: recu.constate_le,
...
async function interroger(requete: Requete): Promise<{ lignes: Ligne[]; total: number }> {
  ...
  const recue = await api.incidentsFilter(
    {
      sort: ordre === null ? undefined : [ordre.sens === 'asc' ? ordre.cle : `-${ordre.cle}`],
      reference: motif === '' ? undefined : { contains: motif },
    },
...
async function lire(cle: string): Promise<Ligne> {
  return depuis(await api.incidentsFind(cle))
}

async function enregistrer(cle: string | null, formulaire: Formulaire): Promise<void> {
  if (cle === null) {
    await api.incidentsCreate(corps(formulaire))
  } else {
    await api.incidentsUpdate(cle, corps(formulaire))
  }
}

async function supprimer(cle: string): Promise<void> {
  await api.incidentsDelete(cle)
}
      ...
      <DialogContent>
        ...
        <form class="flex flex-col gap-5" novalidate @submit.prevent="soumettre">
          <div class="flex flex-col gap-2">
            <Label for="champ-reference">Reference</Label>
            <Input
              id="champ-reference"
              type="text"
          ...
          <div class="flex flex-col gap-2">
            <Label for="champ-sujet">Sujet</Label>
            <Input
              id="champ-sujet"
              type="text"
              :model-value="valeurs.sujet"
              required
              @update:model-value="(valeur) => (valeurs.sujet = String(valeur))"
            />
          </div>
          <div class="flex flex-col gap-2">
            <Label for="champ-detail">Detail</Label>
            <Input
              id="champ-detail"
              type="text"
              :model-value="valeurs.detail"
              required
              @update:model-value="(valeur) => (valeurs.detail = String(valeur))"
            />
          </div>
          <div class="flex flex-col gap-2">
            <Label for="champ-gravite">Gravite</Label>
            <Select
              :model-value="valeurs.gravite"
              @update:model-value="(valeur) => (valeurs.gravite = String(valeur))"
            >
              <SelectTrigger id="champ-gravite">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <div class="flex flex-col gap-2">
            <Label for="champ-ouvert">Ouvert</Label>
            <Checkbox
              id="champ-ouvert"
              :model-value="valeurs.ouvert"
              @update:model-value="(valeur) => (valeurs.ouvert = valeur === true)"
            />
          </div>
          <div class="flex flex-col gap-2">
            <Label for="champ-duree_minutes">Duree minutes</Label>
            <Input
              id="champ-duree_minutes"
              type="number"
              :model-value="valeurs.duree_minutes"
              @update:model-value="(valeur) => (valeurs.duree_minutes = String(valeur))"
            />
          </div>
          <div class="flex flex-col gap-2">
            <Label for="champ-echeance">Echeance</Label>
            <Input
              id="champ-echeance"
              type="date"
              :model-value="valeurs.echeance"
              @update:model-value="(valeur) => (valeurs.echeance = String(valeur))"
            />
          </div>
          <div class="flex flex-col gap-2">
            <Label for="champ-constate_le">Constate le</Label>
            <Input
              id="champ-constate_le"
              type="datetime-local"
              :model-value="valeurs.constate_le"
              required
              @update:model-value="(valeur) => (valeurs.constate_le = String(valeur))"
            />
          </div>
