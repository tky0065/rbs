...
///
/// Le 401 voisin ne dit pas la même chose — l'extracteur refuse avant le handler, la garde
/// refuse dedans. Les confondre laisserait passer un `--role admin` devenu sans effet, la
/// route restant fermée aux anonymes. Le même jeton `user` lit pourtant `/posts` : c'est
/// le seuil, et non l'égalité, que ces deux régimes mettent à l'épreuve.
#[tokio::test]
#[ignore = "joint la base du projet"]
