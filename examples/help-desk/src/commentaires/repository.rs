...
use super::filter::{self, CommentaireFilter};
...
pub async fn list(db: &DatabaseConnection, pagination: &Pagination) -> Result<(Vec<Model>, u64)> {
    ...
    filter(db, &CommentaireFilter::default(), pagination).await
}
...
pub async fn filter(
    ...
    let (commentaires, total) = tokio::try_join!(page, requete.count(db))?;

    Ok((commentaires, total))
}
...
pub async fn create(db: &DatabaseConnection, commentaire: ActiveModel) -> Result<Model> {
    commentaire.insert(db).await.map_err(conflict_on_duplicate)
}

pub async fn update(db: &DatabaseConnection, commentaire: ActiveModel) -> Result<Model> {
    commentaire.update(db).await.map_err(conflict_on_duplicate)
}
