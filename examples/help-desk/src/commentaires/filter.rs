...
pub struct CommentaireFilter {
    ...
    pub corps: Option<TextMatch>,
    #[schema(value_type = Option<rbs_core::UuidComparisonSchema>)]
    pub ticket_id: Option<Comparison<Uuid>>,
    #[schema(value_type = Option<rbs_core::UuidComparisonSchema>)]
...
fn column_of(name: &str) -> Result<Column> {
    Ok(match name {
        ...
        "corps" => Column::Corps,
        "ticket_id" => Column::TicketId,
        "auteur_id" => Column::AuteurId,
        inconnue => {
            return Err(Error::BadRequest(format!(
                "colonne de tri inconnue « {inconnue} » — id, created_at, updated_at, corps, ticket_id, auteur_id"
            )));
        }
    })
}
...
/// dépend.
pub(super) fn apply(select: Select<Entity>, filtre: &CommentaireFilter) -> Result<Select<Entity>> {
    let conditions = Condition::all()
        ...
        .add(matches(Column::Corps, filtre.corps.as_ref()))
        .add(compare(Column::TicketId, filtre.ticket_id.as_ref()))
