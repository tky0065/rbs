...
use super::filter::{self, TicketFilter};
...
pub async fn list(db: &DatabaseConnection, pagination: &Pagination) -> Result<(Vec<Model>, u64)> {
    ...
    filter(db, &TicketFilter::default(), pagination).await
}
...
pub async fn filter(
    ...
    let (tickets, total) = tokio::try_join!(page, requete.count(db))?;
...
pub async fn create(db: &DatabaseConnection, ticket: ActiveModel) -> Result<Model> {
    ticket.insert(db).await.map_err(conflict_on_duplicate)
}

pub async fn update(db: &DatabaseConnection, ticket: ActiveModel) -> Result<Model> {
    ticket.update(db).await.map_err(conflict_on_duplicate)
}
