...
use super::model::{Column, Entity, TicketPriorite, TicketStatut};
...
pub struct TicketFilter {
    ...
    #[schema(value_type = Option<rbs_core::OneOfSchema<TicketStatut>>)]
    pub statut: Option<OneOf<TicketStatut>>,
    #[schema(value_type = Option<rbs_core::OneOfSchema<TicketPriorite>>)]
    pub priorite: Option<OneOf<TicketPriorite>>,
    #[schema(value_type = Option<rbs_core::UuidComparisonSchema>)]
    pub auteur_id: Option<Comparison<Uuid>>,
...
fn column_of(name: &str) -> Result<Column> {
    Ok(match name {
        ...
        "priorite" => Column::Priorite,
        "auteur_id" => Column::AuteurId,
        inconnue => {
            return Err(Error::BadRequest(format!(
                "colonne de tri inconnue « {inconnue} » — id, created_at, updated_at, sujet, detail, statut, priorite, auteur_id"
            )));
        }
    })
}
...
/// dépend.
pub(super) fn apply(select: Select<Entity>, filtre: &TicketFilter) -> Result<Select<Entity>> {
    let conditions = Condition::all()
        ...
        .add(one_of(Column::Statut, filtre.statut.as_ref()))
        .add(one_of(Column::Priorite, filtre.priorite.as_ref()))
        .add(compare(Column::AuteurId, filtre.auteur_id.as_ref()));
