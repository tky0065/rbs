...
const TEXTES = {
  titre: 'Tickets',
  sous_titre: 'Les lignes de la table tickets, lues et écrites par les routes que ce projet expose.',
  filtre: 'Filtrer',
  lignes: 'lignes',
...
function depuis(recu: TicketResponse): Ligne {
  return {
    id: recu.id,
    sujet: recu.sujet,
    detail: recu.detail,
    statut: recu.statut,
    priorite: recu.priorite,
...
async function interroger(requete: Requete): Promise<{ lignes: Ligne[]; total: number }> {
  ...
  const recue = await api.ticketsFilter(
    {
      sort: ordre === null ? undefined : [ordre.sens === 'asc' ? ordre.cle : `-${ordre.cle}`],
      sujet: motif === '' ? undefined : { contains: motif },
    },
...
async function lire(cle: string): Promise<Ligne> {
  return depuis(await api.ticketsFind(cle))
}

async function enregistrer(cle: string | null, formulaire: Formulaire): Promise<void> {
  if (cle === null) {
    await api.ticketsCreate(corps(formulaire))
  } else {
    await api.ticketsUpdate(cle, corps(formulaire))
  }
}

async function supprimer(cle: string): Promise<void> {
  await api.ticketsDelete(cle)
}
    ...
    <Dialog v-model:open="formulaireOuvert">
      <DialogContent>
        ...
        <form class="flex flex-col gap-5" novalidate @submit.prevent="soumettre">
          ...
          <div class="flex flex-col gap-2">
            <Label for="champ-statut">Statut</Label>
            <Select
              :model-value="valeurs.statut"
              @update:model-value="(valeur) => (valeurs.statut = String(valeur))"
            >
              <SelectTrigger id="champ-statut">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <div class="flex flex-col gap-2">
            <Label for="champ-priorite">Priorite</Label>
            <Select
              :model-value="valeurs.priorite"
              @update:model-value="(valeur) => (valeurs.priorite = String(valeur))"
            >
              <SelectTrigger id="champ-priorite">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
