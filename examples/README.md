...
| `newsletter-queue` | `jobs` and `mail`: a broadcast route that enqueues one letter per confirmed subscriber, inside the transaction that reads them. |
...
order the features arrive and must stay sorted, so `jobs`, then `mail`, then a resource
that follows both — which rules out `newsletter` as the resource name. `email` on its own
...
`newsletter-queue` carries eleven more, and they are the point of the example — the
fragment ships a queue and no route, and a job that nothing enqueues proves nothing about
the queue.
