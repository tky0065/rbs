...
fn creation() -> Value {
    ...
    json!({
        "email": format!("email-{suffix}@example.com"),
...
fn modification() -> Value {
    ...
    json!({
        "email": format!("email-modifie-{suffix}@example.com"),
        "name": format!("name-modifie-{suffix}"),
        "confirmed": false,
    })
}
...
async fn an_unknown_sort_column_returns_400() {
    ...
    let (status, body) = call(&api, request("POST", "/subscribers/filter", critere)).await;
...
async fn an_unknown_id_returns_404() {
    ...
    let resource = format!("/subscribers/{inconnu}");
