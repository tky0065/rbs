...
pub fn refusal_in(error: &(dyn std::error::Error + 'static)) -> Option<Refusal> {
    std::iter::successors(Some(error), |cause| {
        // `io::Error::source()` rend la cause de l'erreur qu'il enveloppe, et saute donc
        // celle-ci : un refus relayé dans un `io::Error` ne s'atteint que par `get_ref`.
        cause
            .downcast_ref::<std::io::Error>()
            .and_then(|io| io.get_ref())
            .map(|dedans| dedans as &(dyn std::error::Error + 'static))
