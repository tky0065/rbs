...
async fn post_refuses_a_blocked_target_before_sending() {
    ...
    assert!(
        matches!(
            resultat,
            Err(super::delivery::PostError::Blocked(Refusal::Https))
        ),
        "{resultat:?}"
    );
}
