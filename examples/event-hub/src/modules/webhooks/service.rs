    ...
    T: Serialize + ?Sized,
{
    ...
    for abonnement in repository::actifs(db).await? {
        let motifs: Vec<String> = serde_json::from_value(abonnement.events)?;
...
/// Tous les abonnements, révoqués compris : `revoked_at` n'aurait aucun sens dans une liste
/// qui ne pourrait jamais le porter.
