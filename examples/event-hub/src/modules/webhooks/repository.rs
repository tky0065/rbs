...
/// Les abonnements que rien n'a révoqués — ceux qu'`emit` doit livrer.
///
...
/// Tous les abonnements, révoqués compris — ce que `list` doit montrer.
///
/// La révocation est datée et non destructrice précisément pour que l'historique d'un
/// abonnement reste lisible après elle : le filtrer ici viderait ce que `revoked_at` existe
/// pour dire.
pub async fn tous<C: ConnectionTrait>(db: &C) -> Result<Vec<Model>> {
    Ok(Entity::find()
        .order_by_asc(Column::CreatedAt)
        .all(db)
        .await?)
