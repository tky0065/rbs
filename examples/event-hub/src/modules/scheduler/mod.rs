...
use crate::modules::jobs::{self, Job};
...
///
/// En instructions et non en `vec![]` : une ancre posée à l'intérieur d'une expression — ce
/// `vec![]`, ou la chaîne de `.register()` qu'était `jobs::registry` — ne survit pas à rustfmt
/// dès qu'un second élément s'y ajoute. Clippy préférerait le `vec![]`, seule forme que l'ancre
/// ne peut pas porter.
...
///
/// La réconciliation est faite **ici et non dans la boucle détachée** : elle compile
/// chaque expression, et son échec doit remonter à `main` pour arrêter le démarrage. Une
/// expression illisible détachée ne ferait qu'une ligne de log dans un service qui répond
/// — un calendrier qui ne déclenchera jamais rien, et qu'aucune sonde ne distingue d'un
/// calendrier sain. C'est l'écart assumé avec `jobs::worker::spawn`, où c'est le service
/// HTTP qui est en jeu ; ici, une liste statique que le développeur vient d'écrire.
///
/// Une fois le calendrier validé, la boucle est détachée : au-delà du démarrage, une base
/// momentanément injoignable ne doit plus emporter le serveur. Détachée par le signal
/// d'arrêt de l'état, pour que `main` l'attende avant de sortir.
pub async fn spawn(state: AppState) -> anyhow::Result<()> {
    let schedules = schedules();

    sync::reconcilier(state.core().db(), &schedules).await?;

    let shutdown = state.core().shutdown().clone();
    shutdown.spawn(async move { ticker::run(state, schedules).await });
