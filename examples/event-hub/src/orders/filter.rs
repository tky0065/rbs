...
pub struct OrderFilter {
    ...
    pub amount: Option<Comparison<i32>>,
    ...
    Ok(match name {
        ...
        inconnue => {
            return Err(Error::BadRequest(format!(
                "colonne de tri inconnue « {inconnue} » — id, created_at, updated_at, reference, amount"
            )));
        }
    })
}
...
/// dépend.
pub(super) fn apply(select: Select<Entity>, filtre: &OrderFilter) -> Result<Select<Entity>> {
    let conditions = Condition::all()
        ...
        .add(compare(Column::Amount, filtre.amount.as_ref()));
