...
async fn the_filter_narrows_the_list() {
    ...
    let critere = json!({ "reference": sent["reference"] });
...
async fn an_unknown_sort_column_returns_400() {
    ...
    let (status, body) = call(&api, request("POST", "/orders/filter", critere)).await;
...
async fn an_unknown_id_returns_404() {
    ...
    let resource = format!("/orders/{inconnu}");
