...
- **The README opens on a recording**, from `rbs new` to the admin screen, regenerated by
  one command that fails as soon as one of its own does.
- **A page comparing rbs and [Loco](https://loco.rs)**, on philosophy rather than a feature
  table: where each project draws the line between framework and generated code. Every
  claim about Loco is dated and sourced.
- **An agent scenario**, `scenarios/commentaires/`, run by hand before a minor release: a
  fresh project, a task, and a verdict that only passes if the agent went through the CLI.
  Its first pass is recorded in `ROADMAP.md`.

### Changed

- **Every command output shown by the documentation or the READMEs is now replayed by a
  test**, or declares in the page itself why it cannot be. Several were lying: `rbs add
  auth` writes 51 files and not 40, a SQLite project holds 22 files and not 21, `rbs routes`
  was missing two routes. The counts left in the prose are compared to their source.
- **A generated project is now built on macOS and Windows in CI**, with every feature
  installed. Nothing generated had ever been compiled outside Linux.
