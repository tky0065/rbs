...
- **A generated project now carries a `Makefile`, and no longer depends on its generator.**
  ...
  dependency: a `trap 'kill 0' INT TERM` and a `wait`. Target names are the same in
...
- **`rbs upgrade` writes the `Makefile` on a project generated before it existed.** The file
  is new, so no project generated by an earlier rbs carries one, and nothing it writes can
  conflict with anything — the same reason and the same rule as the `CLAUDE.md` before it: a
  project that already carries one keeps it byte for byte, shortcuts of its own included.
  What lands there is the skeleton's file, in the project's language; `front` and `image`
  stay the business of `rbs add`, which an upgrade does not replay.
...
- **`make dev` no longer kills its own caller when what it runs ends on its own.** Its trap
  covered `EXIT`, and `wait` only returns once every process it started has finished: the
  `kill 0` fired then had nothing left to stop and took the whole process group down —
  make, and the shell of any script that had called the recipe. Measured on a bare project,
  a `make dev` whose binary simply exited died of its own SIGTERM instead of returning zero.
  The trap now covers `INT` and `TERM` only, which is what it was for. A Ctrl-C was already
  correct and stays so: both halves stop, and nothing survives them — that is now run as a
  test rather than argued from the file.
