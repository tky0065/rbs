...
use super::{Schedule, normaliser, sync, ticker};
...
async fn concurrent_boots_all_succeed_and_insert_one_row() {
    ...
    let lignes = Entity::find()
        .filter(super::model::Column::Kind.eq(Scheduled::KIND))
        .all(state.core().db())
        .await
        .expect("lecture possible");
