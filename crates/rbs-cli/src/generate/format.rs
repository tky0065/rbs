...
//! noms. La signature de `list` mesure `95 + len(entite)` — elle tient sur une ligne pour
...
fn formate(source: &str) -> Result<String, Avertissement> {
    ...
    rustfmt
        ...
        .map_err(|erreur| format!("rustfmt n'a pas lu le rendu ({erreur})"))?;

    let sortie = rustfmt
        .wait_with_output()
        .map_err(|erreur| format!("rustfmt n'a pas rendu la main ({erreur})"))?;
    ...
    String::from_utf8(sortie.stdout)
        .map_err(|erreur| format!("rustfmt n'a pas rendu d'UTF-8 ({erreur})"))
}
...
mod tests {
    ...
    #[test]
    fn une_source_deja_formatee_ne_bouge_pas() {
        let source = "fn main() {\n    println!(\"bonjour\");\n}\n".to_string();
        let mut sources = [source.clone()];

        assert_eq!(formate_lot(sources.iter_mut()), None);
        assert_eq!(sources[0], source);
    }

    #[test]
    fn une_source_mal_formatee_est_redressee() {
    ...
    #[test]
    fn une_source_que_rustfmt_refuse_est_rendue_intacte_avec_un_avertissement() {
        let source = "fn main( {".to_string();
        let mut sources = [source.clone()];

        let avertissement = formate_lot(sources.iter_mut()).expect("rustfmt doit se plaindre");
    ...
    #[test]
    fn un_lot_entier_ne_rend_qu_un_avertissement() {
