...
Sixteen routes come with it, on fourteen paths — `/auth/sessions` carries both the listing
...
email link — covered right below. Seven more act on a forgotten password, an
unconfirmed address, or the caller's own sessions, each in its own section further down
this page. The last one, `POST /users/filter`, lists accounts to an administrator: it is
what lets an admin screen show an author by their email, and
[Listing accounts](#listing-accounts) covers it.
...
`POST /users/filter` is the one route of the feature that does not live under `/auth`: it
lists accounts, and only to an administrator. It exists for the admin screens — a reference
to `users` is chosen in a picker and shown by its email, and the picker needs something to
search — so it reads the same body as the filter route `generate crud` writes for a table,
...
or a date. The threshold is `Role::Admin`, because every address the service knows is
personal data, and the admin space itself is open to any session: a lower one would hand
the whole list to whoever signed up. Without a token the route answers 401, below `admin`
403, and a filter it cannot read 400.

```rust file=examples/blog-auth/src/auth/controller/account.rs region=comptes
```

A project whose `auth` was installed before 1.10.0 does not have this route, and `rbs
upgrade` cannot lay it down — it never rewrites a controller. `generate crud` then keeps a
reference to `users` as an identifier input, and its plan points to this section. Adding
the route by hand means taking, from a project generated by 1.10.0 or from
[`examples/blog-auth`](https://github.com/tky0065/rbs/tree/main/examples/blog-auth), one
piece per layer: `UserSummary` and `UserFilter` in `src/auth/dto.rs`; `filter` and the
helpers it calls in `src/auth/repository/user.rs`, re-exported by `repository/mod.rs`;
`filter_users` in `src/auth/service/account.rs`, re-exported by `service/mod.rs`; the
handler above in `src/auth/controller/account.rs`; the line
`.route("/users/filter", post(controller::account::filter_users))` in `src/auth/mod.rs`;
...
`verification.rs`, `guard.rs`, `users.rs` and `openapi.rs`, around the shared harness of `mod.rs` and
...
above, the verified-address guard, the account list and the OpenAPI document.
