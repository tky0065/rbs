...
Installs a feature into an existing project. Eleven are shipped: `auth`, `ci`, `cors`,
`docker`, `jobs`, `mail`, `observability`, `rate-limit`, `redis`, `scheduler` and
`storage`.
...
[above](#the-eleven-features). The rule is the same everywhere: no AST is ever rewritten,
