...
## The project's shortcuts

The skeleton writes a `Makefile`, and `make` on its own prints what it holds. Every recipe
wraps `cargo`, `npm` or `docker compose`, and none of them calls `rbs`: the project is
yours from the moment it is written, and a colleague who clones the repository runs it
without installing the generator first.
...
Shortcuts for demo - make <target>
  ...
  back         run the API alone
  build        build with optimisations
  test         run the tests
  lint         run clippy, warnings included
  fmt          reformat the code
  migrate      apply the pending migrations
  seed         insert the demonstration data
  up           start the services of docker-compose.yml
  down         stop those same services
  openapi      write the OpenAPI document on standard output
...
server to the same list, and `make dev` then serves both.

The `# <rbs:make>` anchor at the foot of the file is where a fragment adds a shortcut of
its own — `front`, `front-build` and `typecheck` for the client, `image` for `docker`. A
fragment adds one only when it brings one more executable to run: `jobs` runs inside the
binary, `observability` sends its traces from it, and neither has a command to wrap.
...
`auth` on its own brings `mail` and `rate-limit` with it, and the report names all three,
each with its own advice: what the pass writes, you read — whether you typed it or not.

The order installed is derived from the fragments rather than from the order they were
typed in. A fragment goes before the ones that require it, and before the ones that insert
into an anchor whose file it writes — `docker` lays down the compose that `mail` extends
with its service — and the name settles the rest, so that two equivalent `--with` render
the same project. Every feature is installed in one pass, so each fragment sees the ones
...
`storage,auth,docker` was typed; `docker` went first, because `mail` — which `auth`
requires — extends the compose it writes; then `mail` and `rate-limit`, ahead of the
`auth` that requires them; `storage`, which nothing constrains, closes the list by name.
...
`--preset` says an intention where `--with` says a list:

| Preset | Features |
|---|---|
| `api` | `auth`, `cors`, `docker`, `rate-limit` |
| `worker` | `docker`, `jobs`, `redis`, `scheduler` |
| `full` | everything the CLI can install |

`full` is derived from what the binary carries rather than written down: a list frozen here
would go stale at the first fragment added, and "everything" would quietly stop meaning
everything.

`api` names `rate-limit` even though `auth` pulls it in anyway. The preset says what it
guarantees, instead of leaning on an implication that could change.

**The two flags add up.** `--preset api --with mail` installs the four of `api` plus `mail`,
without repeating what both name, and in the order of the features rather than the order you
typed — two equivalent invocations must produce two identical projects.
