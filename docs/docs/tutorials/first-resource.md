...
Ten files written with no database running: proof that `--fields` alone was enough to
...
The rebuild is what picks up `src/articles/`: once this line prints, `demo` is listening
with four new routes added to `/health` — `POST`, `GET`, `PATCH` and `DELETE` on
`/articles`, plus `/articles/{id}`.
...
Four of the seven files `generate crud` wrote, each read from
...
layer these seven files land in, including the two this page never opened —
`service.rs` and `filter.rs`.
