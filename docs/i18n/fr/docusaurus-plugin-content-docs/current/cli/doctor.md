...
|---|---|
| `ancres` | Les quatorze ancres Rust en commentaire : `// <rbs:features>` dans `src/lib.rs` — ou dans `src/main.rs`, sur un projet engendré avant que cette bibliothèque existe — `// <rbs:modules>` dans `src/modules/mod.rs`, `// <rbs:routes>` et `// <rbs:layers>` dans `src/router.rs`, `// <rbs:openapi>` dans `src/openapi.rs`, `// <rbs:migration_modules>` et `// <rbs:migrations>` dans `migration/src/lib.rs`, `// <rbs:state_champs>` et `// <rbs:state_init>` dans `src/state.rs`, `// <rbs:startup>` dans `src/main.rs`, `// <rbs:seeds>` dans `src/seeds/main.rs`, `// <rbs:jobs>` dans `src/modules/jobs/mod.rs`, `// <rbs:health_probes>` dans `src/health/controller.rs` — plus l'ancre YAML `# <rbs:services>` dans `docker-compose.yml`. Trois sont optionnelles, inapplicables plutôt qu'absentes quand leur fichier n'existe pas : `modules`, sur un projet qui n'a jamais installé de fragment ; `jobs`, sur un qui ne l'a pas installé ; `services`, sur un sans compose. |
