...
connaît onze — `auth`, `ci`, `cors`, `docker`, `jobs`, `mail`, `observability`,
`rate-limit`, `redis`, `scheduler` et `storage` — et installe chacune des nommées, dans la
même passe qui écrit le projet :
...
erreur : `graphql` n'est pas une feature rbs — disponibles : auth, ci, cors, docker, jobs, mail, observability, rate-limit, redis, scheduler, storage
```
