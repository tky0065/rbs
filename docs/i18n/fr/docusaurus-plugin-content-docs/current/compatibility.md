...
dix-sept ancres en commentaires — seize en Rust, `// <rbs:features>`,
...
dans le bloc `impl HasAuth` de tout projet qui porte l'authentification, plus une en
YAML pour tout projet qui porte un compose, `# <rbs:services>` — et une section `[package.metadata.rbs]` dans `Cargo.toml`, qui
...
juste à côté — le nom de la table tenant lieu du nom fixe que portent les dix-sept
ci-dessus, puisqu'un même fichier peut porter plusieurs entités. Elles sortent du compte
des dix-sept parce que le fichier qui les porte dépend des features du projet, non du
...
- les dix-sept noms d'ancres et leur syntaxe de commentaire ne changent pas, non plus que la
...
vérifie toutes les dix-sept — onze sur un projet qui ne porte ni compose, ni file, ni
fragment déplacé sous `src/modules/`, les six optionnelles — avant que rien n'aille mal.
