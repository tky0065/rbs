...
vingt-deux ancres en commentaires — seize en Rust, `// <rbs:features>`,
...
dans le bloc `impl HasAuth` de tout projet qui porte l'authentification, trois en
TypeScript, qui se commente comme le Rust — `// <rbs:vite_proxy>` dans
`frontend/vite.config.ts`, la liste des préfixes que le serveur de développement relaie au
binaire, pour tout projet qui porte le [client](./guides/frontend.md), et
`// <rbs:admin_routes>` dans `frontend/src/admin/montage.ts` et `// <rbs:admin_rail>` dans
`frontend/src/admin/rail.ts`, pour tout projet qui porte le shell d'administration — plus
trois hors des deux, au marqueur de commentaire `#` de Git : `# <rbs:services>` dans
`docker-compose.yml` pour tout projet qui en porte un, `# <rbs:ignore>` dans
`.gitignore`, où un fragment exclut ce qu'il dépose, et `# <rbs:make>` dans le `Makefile`,
où un fragment ajoute un raccourci à lui — et une section
`[package.metadata.rbs]` dans `Cargo.toml`, qui
consigne la version de rbs ayant engendré le projet, les features qui y sont installées et
la base qu'il vise. Ni l'une ni l'autre n'est une API Rust. Une promesse de compatibilité
écrite pour les seules API Rust enjamberait les deux sans les voir.
...
juste à côté — le nom de la table tenant lieu du nom fixe que portent les vingt-deux
ci-dessus, puisqu'un même fichier peut porter plusieurs entités. Elles sortent du compte
des vingt-deux parce que le fichier qui les porte dépend des features du projet, non du
...
- les vingt-deux noms d'ancres et leur syntaxe de commentaire ne changent pas, non plus que la
...
vérifie toutes les vingt-deux — quatorze sur un projet qui ne porte ni file, ni calendrier,
ni authentification, ni client, ni shell d'administration, onze des vingt-deux étant
optionnelles — avant que rien n'aille mal.
