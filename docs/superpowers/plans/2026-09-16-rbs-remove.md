...
> **Correction du contrôleur (R14) — quatre instructions de ce texte sont périmées.**
> Vérifié dans le dépôt avant le dispatch ; suis cette note plutôt que le texte ci-dessous.
> 1. **Ne touche PAS `templates/agents/{en,fr}.md.jinja`.** La ligne `rbs remove` y est déjà
>    (tâche 10), et les cinq `examples/*/AGENTS.md` la portent. Y retoucher rendrait
>    `integration_examples` rouge. **Vérifie** que c'est bien le cas, ne refais rien.
> 2. **Ne touche PAS `docs/sidebars.ts`** : il ne porte qu'une entrée autogénérée
>    (`{type: 'autogenerated', dirName: '.'}`), et `docs/docs/cli/_category_.json` gouverne la
>    section. Une page neuve y apparaît seule. En revanche les pages CLI portent un
>    `sidebar_position` en tête (`new: 1`, `add: 3`, `doctor: 5`, `seed: 7`, `upgrade: 8`,
>    `completions: 9`…) : ta page neuve en a besoin d'un, choisi de façon cohérente — relève
>    les positions occupées avant de trancher, et vérifie le rendu.
> 3. **`CHANGELOG.fr.md` manque de la liste** : le dépôt tient une jumelle française et la
>    règle bilingue s'applique. Il n'existe aucune section « Unreleased » — la dernière
>    publiée est `[1.5.0] — 2026-09-12` : crée-la, dans les deux langues.
> 4. **La dernière ligne de la passe finale ne t'appartient pas** : cocher `IMPROVE.md` et
>    clore la branche sont le travail du contrôleur. Ne les fais pas.
