...
- [ ] Test rouge dans `agents.rs` : le guide rendu (fr, en) contient `sept fichiers`/`seven files`
...
- [ ] Corriger les deux gabarits : ligne `crud` → huit fichiers (vérifié : mod, model, dto,
  ...
  `auth` après coup.
- [ ] `cargo test -p rbs-cli --lib agents::` vert.
- [ ] Régénérer les quatre `examples/*/AGENTS.md` (zone guide seule) ;
...
- [ ] Test rouge dans `integration_docs.rs` (passe rapide) :
  ...
  workspace).
- [ ] `cargo test -p rbs-cli --test integration_docs the_readmes` → FAIL (1.2.0).
- [ ] Passer les deux README à `Version 1.5.0.` → PASS.
- [ ] Commit `docs(readme): …`.
...
- [ ] Remplacer les deux blocs par des blocs marqués
...
- [ ] « The eleven names » / « Les onze noms » → « These names » / « Ces noms ».
- [ ] `cargo test -p rbs-cli --test integration_docs` vert ; un nom retiré à la main du
  bloc bash doit le rendre rouge (vérifier puis remettre).
- [ ] `cd docs && npm run build` vert. Commit `docs(completions): …`.

### Tâche 29 : ROADMAP à jour

**Fichiers :** `ROADMAP.md` (jalons et tableau d'état).

- [ ] Une section `### v1.2`, `### v1.3`, `### v1.4`, `### v1.5` sous `## Jalons`, un
...
- [ ] Quatre lignes au tableau, dates des tags (`git log -1 --format=%ci v1.2.0`…), 1.5
  « prête, non publiée ».
- [ ] `grep -cE 'webhook|scheduler|audit|client' ROADMAP.md` > 0. Commit `docs(roadmap): …`.

### Tâche 28 : CLAUDE.md et parite.mjs

**Fichiers :** `CLAUDE.md:7-11,70`, `docs/scripts/parite.mjs:36`.

- [ ] `RACINE_MONOLINGUE` += `'IMPROVE_OLD.md'` ; `node docs/scripts/parite.mjs` → 0 écart.
- [ ] `CLAUDE.md` : « État du dépôt » réécrit (six jalons v0.1 → v1.1, puis 1.2 → 1.4
...
- [ ] `cd docs && npm audit fix` (sans `--force`) ; `overrides` `serialize-javascript`
  `^7.0.5` si `npm run build` reste vert, sinon exception écrite.
- [ ] `docs/audit-ci.jsonc` : `high: true`, `skip-dev: true`, `allowlist` des GHSA sans
...
- [ ] Étape `npm audit` dans `docs.yml` après `npm ci`.
- [ ] `.github/dependabot.yml` : `cargo` (`/`), `npm` (`/docs`), `github-actions` (`/`),
  hebdomadaire, groupés ; commentaire : `examples/` et les `feature.toml` restent hors
  de sa portée.
- [ ] `deny.toml` : `[licenses]` = liste réelle du graphe (`cargo deny list`), `[sources]`
...
- [ ] Job `deny` dans `ci.yml`, sur le modèle du job `audit`.
- [ ] Commits `ci(docs): …`, `ci: …`.
